...
public class FrostbiteEffect implements IPercentageHoloDescription, IEventBusListener {
    ...
    public void attackEvent(LivingHurtEvent event) {
        if (hasEffect(event.getSource())) {
            ...
            if (Math.random() < efficiency / 100f) {
                int currentAmplifier = Optional.ofNullable(targetEntity.getEffect(MobEffects.MOVEMENT_SLOWDOWN))
                        .map(MobEffectInstance::getAmplifier)
                        .orElse(-1);

                targetEntity.addEffect(new MobEffectInstance(MobEffects.MOVEMENT_SLOWDOWN, 300, Math.min(currentAmplifier + 1, 2), false, false));
            }
        }
    }
    @Override
    ...
    public ItemEffect getEffect() {
        return ITetraEffect.get("frostbite");
    }
}